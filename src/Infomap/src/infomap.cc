...
void printTree(string s,multimap<double,treeNode>::reverse_iterator it_tM,string *nodeNames,vector<double> &degree,double totalDegree,ofstream *outfile);
...
int main(int argc,char *argv[]){
  ...
  multimap<double,treeNode> treeMap;
  multimap<double,treeNode>::iterator it_tM;
  for(int i=0;i<greedy->Nnode;i++){
    
    int Nmembers = node[i]->members.size();
    treeNode tmp_tN;
    it_tM = treeMap.insert(make_pair(node[i]->degree,tmp_tN));
    for(int j=0;j<Nmembers;j++)
      it_tM->second.members.insert(node[i]->members[j]); 
  ...
  for(multimap<double,treeNode>::reverse_iterator it = treeMap.rbegin(); it != treeMap.rend(); it++){
    ...
    s.append(":");
    printTree(s,it,nodeNames,degree,totalDegree,&outfile);
    k++;
  }
  outfile.close();
  ...
  outfile << "*Edges " << NmapLinks << "\x0D\x0A";
  multimap<int,pair<int,int>,greater<double> > sortedLinks;
  for(int i=0;i<Nmod;i++){
    int Nlinks = node[i]->links.size();
    for(int j=0;j<Nlinks;j++){
      if(i <= node[i]->links[j].first)
        sortedLinks.insert(make_pair(node[i]->links[j].second,make_pair(i+1,node[i]->links[j].first+1)));
    }
  }
  for(multimap<int,pair<int,int>,greater<double> >::iterator it = sortedLinks.begin();it != sortedLinks.end();it++)   
...
void printTree(string s,multimap<double,treeNode>::reverse_iterator it_tM,string *nodeNames,vector<double> &degree,double totalDegree,ofstream *outfile){
  ...
  multimap<double,treeNode>::reverse_iterator it;
  if(it_tM->second.nextLevel.size() > 0){
    ...
    int i = 1;
    for(multimap<double,int>::reverse_iterator mem = sortedMem.rbegin(); mem != sortedMem.rend(); mem++){
